...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Dark X Script | %s fps | %s ms'):format(
        math.floor(FPS),
...
CollarPlayerGroup:AddToggle('ColarPlayer', {
    Text = 'Enabled',
    Default = false,
    Tooltip = "Collar é o jogador que faz com que eles pareçam seu animal de estimação :3",
    Callback = function(Value)
        CollarPlayer = Value
    end
})
...
local AntiAFKButton = AntiAfkGroup:AddButton({
    Text = 'Anti-AFK',
    Func = function()
        Library:Notify("Anti-AFK funcionando")
    ...
    DoubleClick = false,
    Tooltip = 'Desativar kick quando parado'
})

local FreezeCharacterGroup = Tabs.LocalPlayer:AddLeftGroupbox('Congelar personagem')

FreezeCharacterGroup:AddToggle('Congelar personagem', {
    Text = 'Enabled',
    Default = false,
